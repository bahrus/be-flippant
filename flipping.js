export class Flipping {
    ...
    set(key, data) {
        ...
        const a = `translate(${data.inverse.left + data.inverse.top}px, ${data.inverse.top}px) rotate(-90deg) translateY(${data.delta.top}px) rotate(90deg) ${inverseScale}`;
        const aa = `translate(${data.inverse.left + data.inverse.top}px, ${data.inverse.top}px) rotate(0deg) translate(${data.delta.left + data.delta.top}px, ${data.delta.top}px) rotate(0deg) scale(1)`;
        Flipping.style(data, {
            // position
            x: data.rect.left,
            y: data.rect.top,
    ...
    static style(data, styles, options // TODO
    ) {
        const { element } = data;
        if (!element) {
            return;
    ...
    toElementMap(elements) {
        ...
        if (Array.isArray(elements)) {
            elements.forEach(element => {
                const key = typeof this.config.getKey === 'string'
    ...
    flip(elements) {
        const elementMap = this.toElementMap(elements);
        const allKeys = new Set(Object.keys(this.data).concat(Object.keys(elementMap)));
        allKeys.forEach(key => {
            const element = elementMap[key];
            let data;
            ...
            if (!element || !visible) {
                ...
                const delta = existingData.rect
                    ? getDelta(existingData.rect, getRect(element, this.#findAncestor(element)))
                    : NO_DELTA;
                // console.log(existingData, getRect(element), delta);
                data = {
                    key,
                    element,
                    state: existingData.state === 'exit'
                        ? visible
                            ? 'enter'
                            : 'exit'
                        : visible
